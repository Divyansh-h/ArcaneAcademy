...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-gradient-to-br from-purple-50 to-indigo-100 px-4 py-8">
      {/* Animated Background - Optimized */}
      <div className="fixed inset-0 z-0">
        <div className="absolute inset-0 opacity-5 animate-particle noise-bg" />
        <div className="absolute inset-0 bg-gradient-to-br from-purple-100/50 to-indigo-200/50 animate-slow-pulse" />
        <div className="absolute inset-0 bg-gradient-to-tr from-purple-200/20 to-transparent animate-wave" />
        {/* Floating Particles - Reduced count and optimized */}
        {[...Array(5)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-1.5 sm:w-2 h-1.5 sm:h-2 bg-purple-400/50 rounded-full"
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-5xl mx-auto p-8 bg-white/80 backdrop-blur-lg rounded-3xl border border-purple-200 shadow-2xl"
      >
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          ...
          >
            <h1 className="text-3xl font-extrabold text-gray-800 mb-1 flex items-center gap-2">
              <Settings2 className="w-8 h-8 text-purple-600 animate-pulse-slow" />
              Admin Dashboard
            </h1>
            <div className="text-gray-600 text-sm font-mono">Manage teachers, students, subjects, and more</div>
          </motion.div>
          <motion.button
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
        ...
        <div className="flex gap-2 mb-8 relative">
          ...
          {[
            ...
            { label: 'Upload', icon: <Upload className="w-5 h-5" /> },
          ].map((tabItem, index) => (
            <motion.button
              key={index}
        ...
        <div>
          {tab === 0 && (
            <div>
              ...
              <div className="overflow-x-auto rounded-xl border border-purple-200/50 bg-purple-100/30">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-purple-200/50">
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">ID</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Name</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Email</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Subjects</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredTeachers.map((t, i) => (
                      <motion.tr
                        key={i}
                        className="even:bg-purple-200/20"
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.4, delay: i * 0.1 }}
                        whileHover={{ scale: 1.01, boxShadow: '0 0 10px rgba(255, 255, 255, 0.2)' }}
                      >
                        <td className="px-4 py-2 text-gray-800">{t.id}</td>
                        <td className="px-4 py-2 text-gray-800">{t.name}</td>
                        <td className="px-4 py-2 text-gray-800">{t.email}</td>
                        <td className="px-4 py-2">
                          {t.subjects.map((sid) => (
                            <span key={sid} className="inline-block bg-purple-600/60 rounded px-2 py-1 mr-1 text-xs text-gray-200">
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-purple-200/50">
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">ID</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Name</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Section</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Email</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredStudents.map((s, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-gray-800">{s.section}</td>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-purple-200/50">
                      ...
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Name</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Section</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Credits</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Semester</th>
                      <th className="px-4 py-2 text-left text-gray-800 font-mono">Batch</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredSubjects.map((s, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-gray-800">{s.name}</td>
                        <td className="px-4 py-2 text-gray-800">{s.section}</td>
                        <td className="px-4 py-2 text-gray-800">{s.credits}</td>
                        <td className="px-4 py-2 text-gray-800">{s.semester}</td>
                        <td className="px-4 py-2 text-gray-800">{mockBatches.find((b) => b.id === s.batch)?.name || s.batch}</td>
                      </motion.tr>
                    ))}
                  </tbody>
                </table>
          ...
          {tab === 3 && (
            <motion.div
              ...
              transition={{ duration: 0.6 }}
            >
              <div className="text-lg font-bold text-gray-800 mb-2">Batches</div>
              <ul className="list-disc pl-6 space-y-1 text-gray-600">
                {mockBatches.map((b) => (
                  <li key={b.id}>
                    <span className="font-bold text-gray-800">{b.name}</span> ({b.id})
                  </li>
                ))}
              </ul>
            </motion.div>
          ...
          {tab === 4 && (
            <motion.div
              ...
              transition={{ duration: 0.6 }}
            >
              <div className="text-lg font-bold text-gray-800 mb-2">Semesters</div>
              <ul className="list-disc pl-6 space-y-1 text-gray-600">
                {mockSemesters.map((s) => (
                  <li key={s.id}>
                    <span className="font-bold text-gray-800">{s.name}</span> ({s.id})
                  </li>
                ))}
              </ul>
            </motion.div>
          ...
          {tab === 5 && (
            <div>
              ...
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.6, delay: 0.2 }}
                className="mb-8 p-6 rounded-xl bg-purple-100/30 border border-purple-200/50"
              >
                <div className="text-md font-bold text-gray-800 mb-4">Add New Student</div>
                <form onSubmit={handleStudentSubmit} className="space-y-4">
                  <div className="flex flex-col md:flex-row gap-4">
                    <input
                      ...
                      className="flex-1 px-4 py-2 rounded-lg bg-purple-200/50 border border-purple-200/50 text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-400 interactive-card"
                    ...
                    >
                      {mockBatches.map((b, i) => (
                        <motion.option
                          key={b.id}
                          value={b.id}
                          className="bg-purple-300 text-gray-800"
                  ...
                  <motion.button
                    type="submit"
                    className="px-6 py-2 rounded-xl font-bold bg-purple-800 text-gray-200 border border-purple-600 hover:bg-purple-900/70 transition-all interactive-card"
              ...
              <motion.div
                ...
                className="mb-8 p-6 rounded-xl bg-purple-100/30 border border-purple-200/50"
              >
                <div className="text-md font-bold text-gray-800 mb-4">Add New Teacher</div>
                <form onSubmit={handleTeacherSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs mb-1 text-gray-600 font-mono">Assign Subjects</label>
                    <div className="flex flex-wrap gap-2">
                      {filteredSubjects.map((s) => (
                        <motion.label
                          key={s.id}
                          className="flex items-center gap-2 px-3 py-1 rounded-lg bg-purple-200/50 text-gray-800 interactive-card"
                        ...
                        >
                          <input
                            ...
                            className="form-checkbox text-gray-600 focus:ring-purple-400"
                          />
                          {s.name}
                        </motion.label>
                      ))}
              ...
              <motion.div
                ...
                className="p-6 rounded-xl bg-purple-100/30 border border-purple-200/50"
                onDrop={handleFileDrop}
                onDragOver={(e) => e.preventDefault()}
                whileHover={{ boxShadow: '0 0 15px rgba(255, 255, 255, 0.3)' }}
              >
                <div className="text-md font-bold text-gray-800 mb-4">Upload Answer Sheet PDFs</div>
                <div className="text-center text-gray-600 mb-4">
                  <Upload className="w-8 h-8 mx-auto mb-2 animate-pulse-slow" />
                  Drag and drop PDF files here or{' '}
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="underline text-gray-800 hover:text-gray-700 interactive-card"
                  >
                    browse
                  </button>
                </div>
                ...
                {files.length > 0 && (
                  ...
                  >
                    {files.map((file, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between bg-purple-200/50 p-2 rounded-lg"
                      >
                        <span className="text-gray-800 text-sm truncate">{file.name}</span>
                        <button
                          onClick={() => removeFile(i)}
                          className="text-gray-600 hover:text-gray-700 interactive-card"
                        >
                          <X size={16} />
                        </button>
                      </div>
